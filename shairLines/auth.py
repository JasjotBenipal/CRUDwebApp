    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                ...
                return redirect(url_for('endPoints.shairLines'))
            else:
...
def newAccount():
    if request.method == 'POST':
        email = request.form.get('email')
        passwordInitial = request.form.get('passwordInitial')
